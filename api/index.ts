...
export function buildStageStatus(decision: PolicyDecision): StageStatus[] {
  ...
  // Stage 1: Privilege classification - fully implemented with pattern pre-filter + LLM escalation
  stages.push({
    stage: 'privilege',
    status: 'completed',
    ...
    notes:
      reasoningEffort === 'low'
        ? 'Pattern heuristic satisfied. LLM skipped (offline fast path).'
        : `LLM review completed with ${reasoningEffort} reasoning effort.`
  })

  // Stage 2: Responsiveness classification - NOT YET IMPLEMENTED
  // The Python backend has placeholder code that passes through without processing
  stages.push({
    stage: 'responsiveness',
    status: 'not_implemented',
    mode: 'disabled',
    notes: 'Stage 2 (responsiveness classification) is not yet implemented. Awaiting policy template integration.'
  })

  // Stage 3: Redaction span detection - NOT YET IMPLEMENTED
  // The Python backend has placeholder code that passes through without processing
  // Note: PII-based redaction plans ARE generated by the redaction planner, but
  // this stage (LLM-based span detection for privilege redactions) is not implemented
  stages.push({
    stage: 'redaction',
    status: 'not_implemented',
    mode: 'disabled',
    redaction_spans: 0,
    notes: 'Stage 3 (LLM redaction span detection) is not yet implemented. Use PII-based redaction plans instead.'
  })
